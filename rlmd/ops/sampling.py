...
def sample_points_from_polylines(V, L, num_edges, num_samples, return_normals=False):
    """
    Length-weighted uniform sampling of points from a batch of padded polylines.

    Args:
        V: (B, N_max, 2) float — padded vertex positions.
        L: (B, M_max, 2) long — padded edges as index pairs. Pad rows past
            ``num_edges`` are ignored (they may hold any index, including the
            -1 / (0,0) sentinels used by the collate/pad paths).
        num_edges: (B,) long — number of valid edges per item. For a closed
            polyline this equals the vertex count; for welded grids it does not.
